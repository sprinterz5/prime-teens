...
os.environ["DB_PATH"] = "data/test_invites.sqlite3"
...
async def main() -> None:
    ...
    ok = row is not None and row["group_id"] == gid and row["revoked"] == 0
    ...
    row = await db.invite_by_token(token)
    ok = row is not None and row["revoked"] == 1
    ...
    broadcast_targets = {r["tg_user_id"] for r in await db.q(
        "SELECT tg_user_id FROM mentors WHERE is_mentor = 1"
    )}
